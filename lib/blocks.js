...
const blocks = {
    ...
    latestProjects: {
        ...
        queries: [
            {
                key: 'latest',
                query: `*[(_type == 'project') && defined(slug.current)][]{_id, _type, title, slug, "date":coalesce(publishDate, _createdAt), "media":{...media, "asset":media.asset->{_id,path,url,"metadata":{"dimensions":metadata.dimensions, "lqip":metadata.lqip}}}, "category":category->} | order(date desc)`
            }
        ]
    },
    latestPosts: {
        ...
        queries: [
            {
                key: 'latest',
                query: `*[(_type == 'post') && defined(slug.current)][]{_id, _type, title, slug, "date":coalesce(publishDate, _createdAt), "media":{...media, "asset":media.asset->{_id,path,url,"metadata":{"dimensions":metadata.dimensions, "lqip":metadata.lqip}}}, "category":category->} | order(date desc)`
            }
        ]
    },
    columnImages: {
